...
<?php $this->assign('title_text', __('Subcontratos Facturados')); ?>
		...
		<div class="panel panel-warning">
			<div class="panel-heading">
				<div class="row">
					...
					<div class="col-sm-9">
						Resumen de subcontratos facturados para la partida <?= $this->Html->tag('b', $budget_item->item.' - '.$budget_item->description); ?><br><small><i>Esto considera todos los items de los subcontratos que tenga asociado estados de pago y estos mismos además, tengan una factura asociada</i></small>
					</div>
					<?php if($budget_item->budget->currencies[0]->sbif_api_keyword != 'peso'):?>
						<div class="col-sm-2">
					...
					<table class="table tabla-mascara table-hover">
						<thead>
							<tr>
								...
								<th class="text-right info">Fecha del Facturación</th>
								...
								<th class="text-right info">Facturado</th>
								<?php if($budget_item->budget->currencies[0]->sbif_api_keyword != 'peso'):?>
									<th class="text-right warning">Valor de <?= $budget_item->budget->currencies[0]->name;?></th>
									<th class="text-right warning">Total Facturado en <?= $budget_item->budget->currencies[0]->name;?></th>
								<?php endif;?>
							</tr>
						</thead>
						<tbody>
							...
							<?php foreach($subcontratos as $sc):?>
								<?php foreach($sc['EstadoPago'] as $ep):?>
									<tr>
										...
										<td class="text-right"><?= $ep['Data']->ic_factura->NUMFACTURA; ?></td>
										<td class="text-right"><?= $ep['Data']->ic_factura->FECHAEMISION->format('d-m-Y'); ?></td>
										...
										<td class="text-right">
											<?= moneda($ep['Distribucion']['PRECIOVALOR']); ?>
											<?php $grand_total += $ep['Distribucion']['PRECIOVALOR'];?>
										</td>

										<?php
											if($budget_item->budget->currencies[0]->sbif_api_keyword != 'peso')
											{
												$currency_total = $ep['Distribucion']['PRECIOVALOR'] / $ep['Data']['currency_day_value'];
						...
						<table class="table table-hover tabla-mascara">
					    	<thead>
					    		<tr>
					    			...
					    			<th class="text-right">Nº Factura</th>
					    	...
					    	<tbody>
					    		<?php foreach($sc['EstadoPago'] as $ep):?>
						    		<tr>
						    			...
						    			<td class="text-right">
						    				<?php 
						    					if($ep['Data']->ic_factura)
						    					{
						    						echo $ep['Data']->ic_factura->NUMFACTURA; 
						    					}		    					
						    				?>
						    			</td>
						    			<td class="text-right"><?= moneda($ep['Distribucion']['PRECIOVALOR']); ?></td>
						    			<?php
						    				$total += $ep['Distribucion']['PRECIOVALOR'];
						    			?>
						    		</tr>
						    	<?php endforeach;?>
						    	<tr>
