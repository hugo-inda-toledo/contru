 ...
 * Workers Model
 */
class SfWorkerBuildingsTable extends Table
{
    ...
    {
        $this->table('softland.sw_areanegper');
        $this->displayField('ficha');
        $this->primaryKey('ficha', 'codArn');
        $this->hasMany('SfWorkers', [
            'foreignKey' => 'ficha'
        ]);
    }
     ...
     public function beforeFind(Event $event, $query)
    {
        $query->cache(function ($q) {
            ...
            $asdf = 'sfworkersBuildings-' . md5(serialize($tmp));
            return $asdf;
